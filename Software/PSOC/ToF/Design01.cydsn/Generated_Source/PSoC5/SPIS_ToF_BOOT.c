...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPIS_ToF) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)))


/*******************************************************************************
* Function Name: SPIS_ToF_CyBtldrCommStart
********************************************************************************
*
* Summary:
...
void SPIS_ToF_CyBtldrCommStart(void) CYSMALL 
{
    ...
    SPIS_ToF_Start();
    SPIS_ToF_ClearRxBuffer();
    SPIS_ToF_ClearTxBuffer();
}


/*******************************************************************************
* Function Name: SPIS_ToF_CyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIS_ToF_CyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SPIS_ToF_CyBtldrCommWrite
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    (void) SPIS_ToF_ReadTxStatus();

    /* Write TX data using blocking function */
    #if(0u != SPIS_ToF_MODE_USE_ZERO)
        SPIS_ToF_WriteTxDataZero(pData[buf_index]);
        buf_index++;
    #endif /* (SPIS_ToF_MODE_USE_ZERO == 1u) */

    /* Write TX data using blocking function */
    do
    {
        SPIS_ToF_WriteTxData(pData[buf_index]);
        buf_index++;
    }while(buf_index < size);

    while(0u != timeoutMs)
    {
        if(0u != (SPIS_ToF_ReadTxStatus() & SPIS_ToF_STS_SPI_DONE))
        {
            /* Return success code */
            *count = size;
...
/*******************************************************************************
* Function Name: SPIS_ToF_CyBtldrCommRead
********************************************************************************
*
* Summary:
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < (10u * (uint16) timeOut); iCntr++)
    {
        ...
        */
        if(0u != SPIS_ToF_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = SPIS_ToF_GetRxBufferSize();
                CyDelay(SPIS_ToF_BYTE2BYTE_TIME_OUT);
            }while(SPIS_ToF_GetRxBufferSize() > oldDataCount);
            status = CYRET_SUCCESS;
            break;
        }
    ...
    *count = 0u;
    ...
    while(SPIS_ToF_GetRxBufferSize() > 0u)
    {
        tempCount = SPIS_ToF_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = SPIS_ToF_ReadRxData();
                dataIndexCntr++;
            }
...
#endif /* if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_SPIS_ToF) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface)) */
